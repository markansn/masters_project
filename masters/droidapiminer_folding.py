...
def main():
    ...
    Xold, Y, t = load_features("../droidapiminer_data/apg-droidapiminer")

    # for item in os.listdir("../droidapiminer_data/droidapiminer/fixed-x-files"):
    #     print(item)
    #     with open("../droidapiminer_data/x-files/" + item) as f:
    data = []
    X = []
    with open("../droidapiminer_data/apg-droidapiminer-meta.json") as f:
        data = json.load(f)
        for item in data:
            name = item["sha256"]
            try:
                with open("../droidapiminer_data/fixed-x-files2/" + name + "-x.json") as a:
                    entry = {}
                    for item in json.load(a):
                        entry[item] = 1
                    X.append(entry)
            except:
                X.append({})
    ...
    print(len(X))
    print(len(Y))
    f = open("../droidapiminer_data/apg-droidapiminer-X.json", "w+")
    json.dump(X, f)
    f.close()
