...
#
    ...
    # X1, Y1, t1 = load_features("../blade/AA/apg-autopsy") "../droidapiminer_data/apg-droidapiminer"
    X1, Y1, t1 = load_features("../droidapiminer_data/apg-droidapiminer")
    d = {}
    for item in t1:
        # s = float(str(item.year) + "." + str((item.month-1)//3))
        s = float(str(item.year))

        # s = float(str(item.year) + "." + str((item.month)))
        if s not in d:
            d[s] = 1
        else:
            d[s] +=1
